...
function createTimeDisplayElement() {
  ...
  timeDisplayElement.style.top = "5%";
...
timeDisplayElement.style.boxShadow = "0 10px 15px rgb(0 0 0 / 20%)";
  ...
  timeDisplayElement.style.borderRadius = "30px";
...
function updateTimeDisplayPosition() {
  ...
  timeDisplayElement.style.left = "50%";
  timeDisplayElement.style.transform = "translateX(-50%)";
}
...
setInterval(() => {
  chrome.runtime.sendMessage({ type: "updateTime", time: 1000 }, (response) => {
    const timeSpent = formatTime(response.timeSpent);
    const iconPath = chrome.runtime.getURL("timer.png");

    // Use innerHTML to include an image and the time text
    timeDisplayElement.innerHTML = `
    <div style="display: flex; align-items: center;">
    <img src="${iconPath}" alt="Timer Icon" 
